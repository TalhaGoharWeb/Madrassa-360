...
نئے سیٹ اپ کو فوری آزمانے کے لیے ڈیمو ڈیٹا فائل:

- `demo/seed_demo_madrassa.sql` — ایک مکمل نمونہ مدرسہ: طلبہ، اساتذہ، درجات، حاضری، فیس اور امتحانی ڈیٹا۔

**استعمال:**
1. Supabase **SQL Editor** میں پہلے تمام مائیگریشنز لگائیں۔
2. پھر `demo/seed_demo_madrassa.sql` چلائیں۔
3. ڈیمو لاگ اِن سے ایپ آزمائیں۔

**حذف کرنا:** ڈیمو ڈیٹا صاف کرنے کے لیے اسی فائل کے آخر میں دی گئی `CLEANUP` کوری چلائیں
(صرف `is_demo` نشان زدہ ریکارڈ حذف ہوں گے — اصل ڈیٹا محفوظ رہے گا)۔

> اگر `demo/seed_demo_madrassa.sql` ابھی ریپو میں موجود نہ ہو تو یہ سیکشن اس کی
> متوقع شکل بیان کرتا ہے — فائل آتے ہی یہ ہدایات لاگو ہوں گی۔
>
> If `demo/seed_demo_madrassa.sql` is not in the repo yet, this section describes its
> intended shape — the instructions apply as soon as the file lands.
...
- [ ] ڈیمو ڈیٹا سیڈ فائل (`demo/seed_demo_madrassa.sql`)
